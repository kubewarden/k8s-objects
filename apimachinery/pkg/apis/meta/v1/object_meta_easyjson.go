...
func easyjson7cd974a5DecodeGithubComKubewardenK8sObjectsApimachineryPkgApisMetaV1(in *jlexer.Lexer, out *ObjectMeta) {
	...
	for !in.IsDelim('}') {
		...
		case "initializers":
			if in.IsNull() {
				in.Skip()
				out.Initializers = nil
			} else {
				if out.Initializers == nil {
					out.Initializers = new(Initializers)
				}
				(*out.Initializers).UnmarshalEasyJSON(in)
			}
		case "labels":
			if in.IsNull() {
				in.Skip()
...
func easyjson7cd974a5EncodeGithubComKubewardenK8sObjectsApimachineryPkgApisMetaV1(out *jwriter.Writer, in ObjectMeta) {
	...
	if in.Initializers != nil {
		const prefix string = ",\"initializers\":"
		out.RawString(prefix)
		(*in.Initializers).MarshalEasyJSON(out)
	}
	if len(in.Labels) != 0 {
